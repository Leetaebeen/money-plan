...
현재 구현 범위는 외부 서비스와 무관한 TypeScript 계산 엔진과 모바일 우선 PWA입니다. 입력 중인 월급·여윳돈 계획은 브라우저에 자동 저장되며, 사용자가 선택해 저장한 계산 결과의 전체 이력을 다시 열거나 개별 삭제할 수 있습니다. 엔진은 특정 은행 상품, ETF, 종목, 매수 시점이나 예상수익률을 출력하지 않습니다.
...
1. 저장한 계획의 월별 변화 화면 구현
2. PWA 접근성·실사용 베타 검증
3. 공개 계산기 정적 페이지 구현
4. 공식 금융정보 수집기를 별도 패키지로 연결
5. 출처와 기준일이 있는 금융상품 사실조회 화면 구현
